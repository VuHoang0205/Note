...
class VideoCaptureManager private constructor(private val builder: Builder) :
    ...
    private val pendingRecording: PendingRecording by lazy {
        videoCapture.output
            .prepareRecording(getContext(), FileOutputOptions.Builder(getOutput()).build())
    ...
    fun showPreview(previewState: PreviewState, cameraPreview: PreviewView = getCameraPreview()): View {
        getLifeCycleOwner().lifecycleScope.launchWhenResumed {
            ...
            val recorder = Recorder.Builder()
                .setQualitySelector(QualitySelector.from(getQuality()))
                .build()
                .apply {
    ...
    private fun getOutput() = builder.outputFile

    private fun getQuality() = builder.quality

    private fun getLifeCycleOwner() = builder.lifecycleOwner!!

    @SuppressLint("MissingPermission")
    fun startRecording() {
        activeRecording = pendingRecording.start(ContextCompat.getMainExecutor(getContext()), videoRecordingListener)
    }

    fun pauseRecording() {
        if (this::activeRecording.isInitialized) {
    ...
    class Builder(val context: Context, val outputFile: File, val quality: Quality) {
